...
Page({
    ...
    build() {
        ...
        // Register keys: SELECT to start, UP/DOWN to navigate to presets
        onKey({
        ...
        // Register swipe left gesture for presets page
        onGesture({
            callback: (gesture) => {
    ...
    startWorkout() {
        ...
        try {
            ...
            // Add to presets list
            this.addToPresets(this.state.workTime, this.state.restTime, this.state.loopCount)
        } catch (e) {
    ...
    addToPresets(work, rest, loops) {
        try {
            let presets = []
            const saved = this.state.storage.getItem(PRESETS_KEY)
            if (saved) {
                presets = typeof saved === 'string' ? JSON.parse(saved) : saved
                if (!Array.isArray(presets)) {
                    presets = []
                }
            }

            // Check if this exact preset already exists
            const existingIndex = presets.findIndex(p =>
                p.work === work && p.rest === rest && p.loops === loops
            )

            if (existingIndex !== -1) {
                // Move existing preset to top and update timestamp
                const existing = presets.splice(existingIndex, 1)[0]
                ...
                // Remove oldest presets if over limit
                while (presets.length > MAX_PRESETS) {
                    presets.pop()
                }
            }

            this.state.storage.setItem(PRESETS_KEY, JSON.stringify(presets))
            console.log('Preset saved, total:', presets.length)
        } catch (e) {
            console.log('Add preset error', e)
        }
    },
